...
import ShopCardDefault from 'src/sections/hotels/card-item-hotel';
...
export default function HotelView() {
  ...
  const defaultDestination =
    popularDestinations.length > 0
      ...
      : { id: 'default', name: 'Jakarta', CityKey: 'vOoQdQqvHE6Req8ZI8ulZA' };
  ...
  useEffect(() => {
    if (isErrorRecommendationHotels)
      console.error(
        'Error fetching recommendation hotels:',
        recommendationError
      );
  ...
  useEffect(() => {
    if (isReady) {
      ...
      let initLocationKey =
        initialData.locationKey || defaultDestination.CityKey;
      ...
      setSelectedTab(defaultDestination.CityKey);
    }
  }, [isReady, filterData]);
  ...
  useEffect(() => {
    ...
    if (
      selectedItemType &&
      trimmedDebouncedLocation === currentSelectionDisplay
    ) {
      setFilteredDestinations([]);
      return;
    }
  ...
  const handleSelectDestination = (destination) => {
    ...
    const currentHotelName =
      itemType === 'hotel' && destination.HotelName?.trim()
  ...
  const handleDecrement = (setter) => () => {
    setter((prev) =>
      setter === setRooms || setter === setAdults
        ? Math.max(1, prev - 1)
        : Math.max(0, prev - 1)
    );
  };
  ...
  const handleSubmit = () => {
    const checkinStr = values[0]?.isValid
      ? values[0].format('DD-MM-YYYY')
      : null;
    const checkoutStr = values[1]?.isValid
    ...
    if (!selectedItemType || location !== expectedDisplayValue) {
      alert(
        'The destination name seems to have changed. Please select a valid destination from the suggestions again.'
      );
      handleSearchDialogOpen();
      return;
    }
  ...
  const handleTabChange = (event, newValue) => {
    ...
    const selectedCity = popularDestinations.find(
      (dest) => dest.CityKey === newValue
    );
    if (selectedCity) {
    } else {
      console.warn(
        'Could not find popular destination details for CityKey:',
        newValue
      );
    }
  };
  ...
  return (
    <Container sx={{ mb: 10, mt: { xs: -8, md: -10 }, px: { xs: 0, md: 3 } }}>
      <Box
        sx={{
          position: 'relative',
        ...
        <Box
          ...
          <Typography
            ...
            Cari sesuai destinasi dan tanggal sesuai keinginan kamu{' '}
          </Typography>
        </Box>
        <SearchHome
          location={location}
      ...
      <Box sx={{ mt: { xs: 5, md: 7 }, padding: { xs: '0 0 0 1rem', md: 0 } }}>
        <Typography
          variant='h6'
          sx={{
          ...
          Hotel Populer Menarik di Kota Tujuan Kamu
        </Typography>
        <Tabs
          value={selectedTab}
          ...
          sx={{
            ...
            '& .MuiTab-root:not(:last-of-type)': { marginRight: '4px' },
            '& .Mui-selected': {
          ...
          {popularDestinations.map((destination) => (
            <Tab
              ...
              value={destination.CityKey}
            />
          ))}
        </Tabs>
      </Box>
      <Grid container spacing={2} sx={{ mt: 0.5, px: { xs: 2, md: 0.5 } }}>
        {isLoadingRecommendationHotels ? (
          ...
          <Grid item xs={12}>
            <Typography color='error' align='center' sx={{ py: 5 }}>
              Maaf, terjadi kesalahan saat memuat hotel populer. Coba lagi
              nanti.
            </Typography>
          </Grid>
        ) : recommendationHotels?.data?.Hotels &&
          recommendationHotels.data.Hotels.length > 0 ? (
          recommendationHotels.data.Hotels.slice(0, 4).map((item) => (
            <Grid item xs={6} md={3} key={item.HotelKey}>
              <ShopCardDefault
                data={item}
