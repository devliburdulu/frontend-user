...
import { getProductByCat1 } from "src/rest/Product";
import Iconify from "src/components/iconify/iconify";
import { useSnackbar } from "notistack";
import { getHomeBanners } from "src/rest/HomeBanner";

export default function PaketWisataView() {
  const [title, setTitle] = useState("Catalog - Liburdulu.id");
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const result = await getProductByCat1(filter.page, 12, {});
        setProduct(result);
      } catch (error) {
        console.error(error);
      } finally {
